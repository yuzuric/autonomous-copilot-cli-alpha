...
`autonomous-copilot-cli-alpha` is built on top of [Xiaomi MiMo](https://platform.xiaomimimo.com), the open-source large language model series from Xiaomi. The project demonstrates how to integrate MiMo into production systems using its OpenAI-compatible API.
...
```python
import asyncio
from autonomous_copilot_cli_alpha.client import MiMoClient
...
```
┌─────────────┐     ┌──────────────┐     ┌─────────────────┐
│  Application │───▶│  autonomous_cop │───▶│  Xiaomi MiMo    │
│              │     │  (this repo) │     │  (LLM API)      │
└─────────────┘     └──────────────┘     └─────────────────┘
                            │
                            ▼
...
├── config.yaml
├── main.py
├── src/autonomous_copilot_cli_alpha/
│   ├── reviewer.py
│   ├── client.py
│   └── config.py
├── tests/
